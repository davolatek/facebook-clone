...
const Contacts = ({ name, src, status }) => {
  return (
    <div className="flex items-center space-x-2 py-2 pl-1 hover:bg-gray-200 rounded-l-xl cursor-pointer relative">
      ...
      {status == "Online" && (
        <div className="bg-green-500 h-4 rounded-full w-4 absolute left-5 bottom-2 border-2"></div>
      )}
      {status == "Offline" && (
        <div className="bg-gray-500 h-4 rounded-full w-4 absolute left-5 bottom-2 border-2"></div>
      )}
    </div>
  );
};
